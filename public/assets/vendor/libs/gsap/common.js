...
---------------------------------------------------*/
	...
	window.ScrollEffects = function() {
		...
		if (isMobile()) {
			...
			$('.smooth-scroll main, .has-parallax, nav, .kenndeclouv-slider-wrapper:not(.content-slider), .showcase-lists .kenndeclouv-sync-slider, .next-project-image-wrapper, .slider-fixed-content').css({'height': winHeight});
		...
		if ($('#kenndeclouv-page-content').hasClass("light-content")) {
			if ($('#project-nav').hasClass("change-header")) {
		  		imagesLoaded('body', function() {
					setTimeout(function() {	
						var pageNav = document.querySelector('#project-nav.change-header');
						if (pageNav) {
							var pageContent = document.querySelector('#kenndeclouv-page-content');	
							var nextPageCaptionParallax = gsap.to('.page-nav-caption', {
								scrollTrigger: {
									trigger: pageNav,
									start: "top 8%",
		...
		if ($('#kenndeclouv-page-content').hasClass("dark-content")) {
			if ($('#project-nav').hasClass("change-header")) {
		  		imagesLoaded('body', function() {
					setTimeout(function() {	
...
---------------------------------------------------*/	
	...
	window.FirstLoad = function() {
		...
		if ($("#kenndeclouv-page-content").hasClass("light-content")) {
			...
			gsap.to("main", {duration: 0.5, backgroundColor: document.querySelector("#kenndeclouv-page-content").getAttribute("data-bgcolor"), ease:Power2.easeInOut});
		...
		$('a.ajax-link').on('click', function() {
			...
			gsap.to("nav", {duration: 0.3, backgroundColor: document.querySelector("#kenndeclouv-page-content").getAttribute("data-bgcolor")});
					...
					if (evt.type === 'mouseup') {
					  */
					  	$('#itemsWrapperLinks .trigger-item-link, #itemsWrapperLinks .trigger-item-link-secondary').on('click', function() {					
							...
							if (!$('#kenndeclouv-page-content').hasClass("light-content")) {
								...
								if (!$('.portfolio').hasClass("portfolio-shortcode")) {
									if (!parent_item.hasClass("change-header")) {
										$('#kenndeclouv-page-content').delay(1200).queue(function(next){							
							...
							$('.kenndeclouv-slider .trigger-item').each(function(){
		...
		if (!$("body").hasClass("disable-ajaxload")) {
			...
			if ($("#itemsWrapper").hasClass("no-fitthumbs")) {
				...
				$('#itemsWrapperLinks .trigger-item-link').on('click', function() {
				  	...
				  	const fadeOutTargets = $('#hero, #show-filters, .kenndeclouv-slider, .item, .item-caption-wrapper, #page-nav, footer, .fadeout-element');				  	
				  	const ball = $('#ball');
...
---------------------------------------------------*/
	...
	window.Sliders = function() { 
		...
		if( $('.content-slider').length > 0 ){
			...
			slider = new KenndeclouvSlider('.content-slider', { 
			...
			$(".slider-button-prev").mouseenter(function(e) {
				if ($(this).parents(".kenndeclouv-slider-wrapper").hasClass("light-cursor")) {
					$("body").addClass("drag-cursor-white");
					gsap.to('#ball', {duration: 0.2, borderWidth: '2px', scale: 1, borderColor:'#fff'});
				} else if ($(this).parents(".kenndeclouv-slider-wrapper").hasClass("dark-cursor")) {
			...
			$('.content-slider.looped-carousel .kenndeclouv-slider, .content-slider.small-looped-carousel .kenndeclouv-slider').on('mouseenter mousemove', function() {	
...
---------------------------------------------------*/
	...
	window.Lightbox = function() {
		...
		function showDetails(item) {
			...
			$('body').prepend(`<div class="kenndeclouv-img-popup">
				<div class="kenndeclouv-img-popup-bg-close"></div>				
				<div class="kenndeclouv-img-popup-viewport">
					<div class="kenndeclouv-img-popup-preloader"><div></div><div></div><div></div><div></div></div>					
					<img />
					<div class="kenndeclouv-img-popup-close link"></div>						
				</div>				
				<div class="kenndeclouv-img-popup-prev link"></div>
				<div class="kenndeclouv-img-popup-next link"></div></div>`);
			
			const details = document.querySelector('.kenndeclouv-img-popup');
			const detailsBgClose = document.querySelector('.kenndeclouv-img-popup-bg-close');
			const detailsClose = document.querySelector('.kenndeclouv-img-popup-close');
			const detailsPreloader = document.querySelector('.kenndeclouv-img-popup-preloader');
			const detailImage = document.querySelector('.kenndeclouv-img-popup img');
			const detailPrev = document.querySelector('.kenndeclouv-img-popup-prev');
			const detailNext = document.querySelector('.kenndeclouv-img-popup-next');
		...
		function showVideoDetails(event, item) {
			...
			$('body').prepend(`<div class="kenndeclouv-video-popup">
				<div class="kenndeclouv-video-popup-bg-close"></div>				
				<div class="kenndeclouv-video-popup-viewport">
					<div class="kenndeclouv-video-popup-preloader"><div></div><div></div><div></div><div></div></div>					
					<iframe class="kenndeclouv-video-popup-iframe" frameborder="0" allow="autoplay"></iframe>
					<div class="kenndeclouv-video-popup-close link"></div>						
				</div>				
				<div class="kenndeclouv-img-video-prev link"></div>
				<div class="kenndeclouv-img-video-next link"></div></div>`);
			
			const details = document.querySelector('.kenndeclouv-video-popup');
			const detailsBgClose = document.querySelector('.kenndeclouv-video-popup-bg-close');
			const detailsClose = document.querySelector('.kenndeclouv-video-popup-close');
			const detailsPreloader = document.querySelector('.kenndeclouv-video-popup-preloader');
			const detailIframe = document.querySelector('.kenndeclouv-video-popup iframe');
		...
		function hideVideoDetails() {
			...
			gsap.to(detailsBgClose, {duration: 0.5, delay:0.2, backgroundColor:"rgba(0,0,0,0)", onComplete: function() {
					$('.kenndeclouv-video-popup').remove();
				}}),
...
---------------------------------------------------*/
	...
	window.Core = function() {
		...
		if (!isMobile() && !$('body').hasClass("disable-cursor")) {
			...
			$(".kenndeclouv-button .sticky.left").mouseenter(function(e) {		  
				var rcBounds = $(this)[0].getBoundingClientRect();		  
			...
			$(".kenndeclouv-button .sticky.right").mouseenter(function(e) {		  
				var rcBounds = $(this)[0].getBoundingClientRect();		  
			...
			$(".kenndeclouv-button .parallax-wrap.icon-wrap").mouseenter(function(e) {
			...
			$(".kenndeclouv-button .sticky").mouseenter(function(e) {
				if ($('#kenndeclouv-page-content').hasClass("light-content")) {
			...
			$(".kenndeclouv-button .parallax-wrap").mouseenter(function(e) {
				if ($('#kenndeclouv-page-content').hasClass("light-content")) {
			...
			//Blog Hover Effects			
			$("#blog-page-nav .page-numbers li a, .post-page-numbers, #post-content a, #post-form a, #post-comments a, .wp-block-search__button, .kenndeclouv-sidebar-widget a").mouseenter(function(e) {	
				gsap.to('#ball', {duration: 0.2, borderWidth: '1px', scale: 1, opacity:0});
			});	
					
			$("#blog-page-nav .page-numbers li a, .post-page-numbers, #post-content a, #post-form a, #post-comments a, .wp-block-search__button, .kenndeclouv-sidebar-widget a").mouseleave(function(e) {
				gsap.to('#ball', {duration: 0.3, borderWidth: '4px', scale:0.5, opacity:1});
			});
		}
			  ...
			  $(window).on('popstate', function() {
				...
				section.load(url+' .cd-main-content > *', function(event){
				  // load new content and replace <main> content with the new one
				 	...
				 	var _title = event.match(/<title[^>]*>([^<]+)<\/title>/)[1];
