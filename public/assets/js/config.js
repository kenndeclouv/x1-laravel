...
let semiDark;
let skin;
fetch("/api/app-setting")
    .then((response) => response.json())
    .then((data) => {
        primaryColor = data.app_color;
        theme = data.app_theme;
        semiDark = data.app_semidark;
        skin = data.app_skin;
        ...
        if (typeof TemplateCustomizer !== "undefined") {
            window.templateCustomizer = new TemplateCustomizer({
                ...
                defaultPrimaryColor: primaryColor ?? "#7367F0",
                defaultSkin: skin ?? 0,
