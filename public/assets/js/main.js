...
//   const searchJson = '/api/search-data';
...
document.querySelectorAll(".menu-link").forEach(link => {
  link.addEventListener("click", () => {
      let menuName = link.querySelector("div[data-i18n]")?.innerText || "Unknown";
      let iconEl = link.querySelector(".menu-icon"); // ubah ke parentnya
      let menuIcon = iconEl ? [...iconEl.classList].find(cls => cls.startsWith("tabler-")) || "" : "";
      let menuUrl = link.href;

      console.log("ICON CLASS:", iconEl?.classList); // debug buat cek classnya

      let menuClicks = JSON.parse(sessionStorage.getItem("menuClicks") || "[]");

      let menuIndex = menuClicks.findIndex(m => m.url === menuUrl);
      if (menuIndex > -1) {
          menuClicks[menuIndex].count += 1;
      } else {
          menuClicks.push({ name: menuName, icon: menuIcon, url: menuUrl, count: 1 });
      }

      sessionStorage.setItem("menuClicks", JSON.stringify(menuClicks));
  });
});



function getSuggestions(limit = 3) {
  let menuClicks = JSON.parse(sessionStorage.getItem("menuClicks") || "[]");
  menuClicks.sort((a, b) => b.count - a.count); // urutin berdasarkan count
  return menuClicks.slice(0, limit);
}
